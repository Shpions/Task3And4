...
public class TestInnerUnion {
    ...
    public void test_NormalValues() throws Exception {
        Pojo[] result = {new Pojo("Andrey", 18), new Pojo("Slava", 44)};
        Pojo[] test = m.innerUnion(p1, p2);
        Arrays.sort(test);
        Arrays.sort(result);
    ...
    @Test
     public void test_FirstMasEmpty() throws Exception {
        Pojo [] result = pEmpty.clone();
        Pojo [] test = m.innerUnion(pEmpty, p1);
        Arrays.sort(test);
        Arrays.sort(result);
    ...
    public void test_SecoundtMasEmpty() throws Exception {
        Pojo [] result = pEmpty.clone();
        Pojo [] test = m.innerUnion(p1, pEmpty);
        Arrays.sort(result);
        Arrays.sort(test);
    ...
    public void test_FirstMasNull() throws Exception {
        Pojo [] result = pEmpty.clone();
        Pojo [] test = m.innerUnion(pNull, p1);
        Arrays.sort(result);
        Arrays.sort(test);
    ...
    public void test_SecoundMasNull() throws Exception {
        Pojo [] result = pEmpty.clone();
        Pojo [] test = m.innerUnion(p1, pNull);
        Arrays.sort(result);
        Arrays.sort(test);
        assertArrayEquals(test, result);
